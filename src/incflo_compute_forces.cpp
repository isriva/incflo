...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(vel_forces,TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
            ...
            Array4<Real const> const& gradp0 = m_leveldata[lev]->gp0.const_array(mfi);
            Array4<Real const> const& rho0 = m_leveldata[lev]->density0.const_array(mfi);
                ...
                amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
                {
                    ...
                    if (include_pressure_gradient)
                    {
                        if (!use_base_gradp) {
                            ...
                            AMREX_D_TERM(vel_f(i,j,k,0) = -(gradp(i,j,k,0)-gradp0(i,j,k,0))*rhoinv + l_gravity[0];,
                                         vel_f(i,j,k,1) = -(gradp(i,j,k,1)-gradp0(i,j,k,1))*rhoinv + l_gravity[1];,
                                         vel_f(i,j,k,2) = -(gradp(i,j,k,2)-gradp0(i,j,k,2))*rhoinv + l_gravity[2];);
                        }
                    } else {
