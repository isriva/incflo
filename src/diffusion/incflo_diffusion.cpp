...
Array<MultiFab,AMREX_SPACEDIM>
incflo::average_nodal_velocity_eta_to_faces (int lev, MultiFab const& nodal_eta, int order, bool use_harmonic) const
{
    ...
    // amrex::average_cellcenter_to_face(GetArrOfPtrs(r), cc_eta, Geom(lev));
#else
    average_nodal_to_face(r, nodal_eta, order, use_harmonic);
#endif
...
void
incflo::average_nodal_to_face (Array<MultiFab,AMREX_SPACEDIM>& fc, MultiFab const& nodal, int order, bool use_harmonic) const
{
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(fc[0],mfi_info); mfi.isValid(); ++mfi) {
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) {
            if (!use_harmonic) {
                facex(i,j,k) = amrex::Real(0.25)*(node(i,j,k,order) + node(i,j+1,k,order) + node(i,j,k+1,order) + node(i,j+1,k+1,order));
            }
            else {
                facex(i,j,k) = amrex::Real(4.0)/((amrex::Real(1.0)/node(i,j,k,order)) + (amrex::Real(1.0)/node(i,j+1,k,order))
                                                + (amrex::Real(1.0)/node(i,j,k+1,order)) + (amrex::Real(1.0)/node(i,j+1,k+1,order)));
            }
        },
        [=] AMREX_GPU_DEVICE (int i, int j, int k) {
            if (!use_harmonic) {
                facey(i,j,k) = amrex::Real(0.25)*(node(i,j,k,order) + node(i+1,j,k,order) + node(i,j,k+1,order) + node(i+1,j,k+1,order));
            }
            else {
                facey(i,j,k) = amrex::Real(4.0)/((amrex::Real(1.0)/node(i,j,k,order)) + (amrex::Real(1.0)/node(i+1,j,k,order))
                                                + (amrex::Real(1.0)/node(i,j,k+1,order)) + (amrex::Real(1.0)/node(i+1,j,k+1,order)));
            }

        },
        [=] AMREX_GPU_DEVICE (int i, int j, int k) {
            if (!use_harmonic) {
                facez(i,j,k) = amrex::Real(0.25)*(node(i,j,k,order) + node(i+1,j,k,order) + node(i,j+1,k,order) + node(i+1,j+1,k,order));
            }
            else {
                facez(i,j,k) = amrex::Real(4.0)/((amrex::Real(1.0)/node(i,j,k,order)) + (amrex::Real(1.0)/node(i+1,j,k,order))
                                                + (amrex::Real(1.0)/node(i,j+1,k,order)) + (amrex::Real(1.0)/node(i+1,j+1,k,order)));
            }
    ...
    for (MFIter mfi(fc[0],mfi_info); mfi.isValid(); ++mfi) {
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) {
            if (!use_harmonic) facex(i,j,k) = amrex::Real(0.5)*(node(i,j,k,order) + node(i,j+1,k,order));
            else facex(i,j,k) = amrex::Real(2.0)/((amrex::Real(1.0)/node(i,j,k,order)) + (amrex::Real(1.0)/node(i,j+1,k,order)));
        },
        [=] AMREX_GPU_DEVICE (int i, int j, int k) {
            if (!use_harmonic) facey(i,j,k) = amrex::Real(0.5)*(node(i,j,k,order) + node(i+1,j,k,order));
            else facey(i,j,k) = amrex::Real(2.0)/((amrex::Real(1.0)/node(i,j,k,order)) + (amrex::Real(1.0)/node(i+1,j,k,order)));
        });
    }
#endif
}
